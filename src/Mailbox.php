...
final class Mailbox implements MailboxInterface
{
     ...
     */
    public function getMessages(ConditionInterface $search = null, int $sortCriteria = null, bool $descending = false, string $searchCharset = 'UTF-8'): MessageIteratorInterface
    {
        if (null === $search) {
            $search = new All();
        }
        ...
        if (null !== $sortCriteria) {
            ...
            $messageNumbers = \imap_search($this->resource->getStream(), $query, \SE_UID, $searchCharset);
        }
